...
router.post("/api/seed-sample-building", async (req, res) => {
  try {
...
router.put("/api/assessments/:id/verification", async (req, res) => {
  try {
    const id = req.params.id;
...
router.get("/api/assessments/:id/logs", async (req, res) => {
  try {
    const id = req.params.id;
    const [p] = await db.select().from(schema.permohonanPenilaian).where(eq(schema.permohonanPenilaian.idPermohonan, id)).limit(1);
    if (!p) return res.status(404).json({ error: "Permohonan tidak ditemukan" });
...
router.put("/api/assessments/bulk-status", async (req, res) => {
  try {
...
router.put("/api/assessments/:id/status", async (req, res) => {
  try {
    const id = req.params.id;
...
router.put("/api/assessments/:id/disposisi", async (req, res) => {
  try {
    const id = req.params.id;
...
router.put("/api/assessments/:id", async (req, res) => {
  try {
    const id = req.params.id;
